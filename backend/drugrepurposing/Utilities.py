...
def drug_scoring(disease, known_drug, model_path, drug_disease):
    ...
    # Step 5: Create a set of drugs associated with the target disease(s) from drug_disease dataset
    associated_drugs = []
    if disease is not None:
        if isinstance(disease, list):
            # If disease is a list, get all associated drugs for each disease and combine the results
            associated_drugs = drug_disease[drug_disease['DiseaseName'].isin(disease)]['ChemicalID'].tolist()
        else:
            # If disease is a single string, proceed as usual
    ...
    associated_drugs_indices = [drug_to_index[drug] for drug in associated_drugs] if associated_drugs else []
